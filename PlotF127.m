...
exp = readtable('F127 exp param.xlsx');
...
UCL_Rg = data_{7}(1,3) + data_{7}(1,3) * 0.05;
LCL_Rg = data_{7}(end,3) - data_{7}(end,3) * 0.05;
...
hold on
scale_Rc = lsqnonlin(@(scale)fitscalingfactor(frac, Rc_error_bootstrap , scale), 0.1); 
%scale = 0.35;
...
%scale = 1./sqrt(0.01)/Rg_error_bootstrap(1);
