function [params, I_output] = correlation(M_data, I_rep_mat)
%% Input
...
model = 'correlation'; % correlation,glgel,fsgel
...
buffer = importdata('Buffer_merged.txt');
buffer = buffer.data;
...
if strcmp(model,'correlation')
    ...
    x0 = [x_low(1); x_low(2); 0.5; 2; 0.05; 17]; % [Inverse Angstrom q and cm I]
    lb = [x_low(1); x_low(2); 0; 0; 0.01; 0]; 
    ub = [x_low(1); x_low(2); inf; 5; 0.07; 100];
    % here we are doing the full fitting not piecewise
%     x0 = [1e-5; 4; 0.5; 2; 0.05; 17]; % [Inverse Angstrom q and cm I]
%     lb = [0; 0; 0; 0; 0.01; 0]; 
%     ub = [0.1; 10; inf; 5; 0.07; 100];
    ...
    error_ci = ci(:,2) - x;
    MSR = sum((residual.*dI_sub_fit).^2)/length(residual);
    
    params.x_low = x_low;
    params.x = x;
    params.x_high = x_high;
    params.error_ci = error_ci;
    params.MSR = MSR;
...
% 
    ...
    bkg = bkg_avg*ones(1,length(logspace(-3,0)));
    
    I_output.qfit = q_fit;
    I_output.qcut = qcut;
    I_output.I_fit = I_fit;
    I_output.I_fit_lowq = I_fit_lowq;
    I_output.I_fit_midq = I_fit_midq;
    I_output.I_fit_highq = I_fit_highq;
    I_output.I_fit_bkg = bkg;
    I_output.I_sub_solv = I_sub_solv;
    I_output.dI_sub_solv = dI_sub_solv;
...
scale = params(1); L = params(2);
I_fit = scale*L^3/(1+(q*L)^2)^2;
end


%% Ornstein-Zernike and pseudo-Voigt Lorentzian
% I(q) = a(c/(1+q-q1)^2*xi^2 + (1-c) exp(- (q-q1)^2*xi^2/2))
function I_fit = OZVL(q, params)
a = params(1); c = params(2); q1 = params(3); xi = params(4);
I_fit = a(c/(1+q - q1)^2*xi^2 + (1-c) * exp(- (q - q1)^2 * xi^2/2));
end
end